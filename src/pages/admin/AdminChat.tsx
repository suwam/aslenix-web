    ...
    <AdminShell title="Messages">
      <div className="grid grid-cols-12 gap-4 h-[75vh]">
        <div className="col-span-4 gradient-border glass rounded-2xl overflow-hidden flex flex-col">
          <div className="p-3 border-b border-foreground/5 text-xs uppercase tracking-wider text-muted-foreground">Conversations</div>
          <div className="flex-1 overflow-auto">
            {convs.length === 0 && <div className="p-8 text-center text-sm text-muted-foreground">No conversations yet</div>}
            {convs.map((c) => (
              <button key={c.id} onClick={() => setActive(c)} className={`w-full text-left p-3 border-b border-foreground/5 hover:bg-foreground/5 ${active?.id === c.id ? "bg-foreground/10" : ""}`}>
                <div className="flex items-center justify-between">
                  <span className="font-medium text-sm">{c.clients?.company_name ?? c.subject}</span>
                  {c.unread_admin > 0 && <span className="text-[10px] px-1.5 py-0.5 rounded-full bg-brand-gradient text-foreground">{c.unread_admin}</span>}
                </div>
        ...
        <div className="col-span-8 gradient-border glass rounded-2xl flex flex-col">
          {!active ? (
            ...
            <>
              <div className="p-3 border-b border-foreground/5"><div className="font-medium">{active.clients?.company_name ?? active.subject}</div></div>
              <div ref={scroller} className="flex-1 overflow-auto p-4 space-y-3">
                {messages.map((m) => (
                  <div key={m.id} className={`flex ${m.sender_role === "admin" ? "justify-end" : "justify-start"}`}>
                    <div className={`max-w-[70%] px-3 py-2 rounded-2xl text-sm ${m.sender_role === "admin" ? "bg-brand-gradient text-foreground" : "bg-foreground/10"}`}>
                      <div>{m.body}</div>
